...
class _TermsAndConditionsPageState extends State<TermsAndConditionsPage> {
  bool isEmailMarketingAgreed = false;
  bool isSMSMarketingAgreed = false;
  bool isAllAgreed = false;
  bool isRequiredTermsAgreed = false;
  ...
  void _toggleAllAgreements(bool? value) {
    setState(() {
      isAllAgreed = value!;
      isEmailMarketingAgreed = value;
      isSMSMarketingAgreed = value;
      isRequiredTermsAgreed = value;
      isPrivacyPolicyAgreed = value;
      showWarning = !value; // Hide warning if all agreements are toggled
    });
  }

  void _toggleRequiredAgreement(bool? value) {
    setState(() {
      isRequiredTermsAgreed = value!;
      if (value && isPrivacyPolicyAgreed) {
        showWarning = false; // Hide warning if both required agreements are checked
      }
    });
  }
  ...
  void _togglePrivacyPolicyAgreement(bool? value) {
    setState(() {
      isPrivacyPolicyAgreed = value!;
      if (value && isRequiredTermsAgreed) {
        showWarning = false; // Hide warning if both required agreements are checked
      }
    });
  }

  void _toggleEmailMarketingAgreement(bool? value) {
    setState(() {
      isEmailMarketingAgreed = value!;
  ...
  void _toggleSMSMarketingAgreement(bool? value) {
    setState(() {
      isSMSMarketingAgreed = value!;
      if (!value) isAllAgreed = false;
    });
  ...
  void _onAgreePressed() {
    if (!isRequiredTermsAgreed || !isPrivacyPolicyAgreed) {
      setState(() {
        showWarning = true;
      });
    } else {
      ...
      appBar: PreferredSize(
        ...
        child: Center(
          child: Container(
            ...
            child: AppBar(
              title: const Text("회원가입 약관", style: TextStyle(color: Colors.black),),
            ),
          ),
        ),
      ),
                        ...
                        child: ExpansionTile(
                          title: Row(
                            children: [
                              ...
                              Checkbox(
                                value: isRequiredTermsAgreed,
                                onChanged: _toggleRequiredAgreement,
                              ),
                            ],
                          ),
                          children: <Widget>[
...
제1조(목적) 이 약관은 에스앤이컴퍼니 회사(전자상거래 사업자)가 운영하는 비굿 구매대행 사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 구매대행 사이버 몰과 이용자의 권리․의무 및 책임사항을 규정함을 목적으로 합니다.

※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다.」

제2조(정의)

① “몰”이란 에스앤이컴퍼니 회사가 재화 또는 용역(이하 “재화 등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 재화 등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
...
1. 재화 또는 용역에 대한 정보 제공 및 구매계약의 체결 및 대행
2. 구매계약이 체결된 재화 또는 용역의 배송 및 구매대행
3. 기타 “몰”이 정하는 업무
...
① “몰”은 이용자로부터 공급사가 재화 등을 반환받은 경우 3영업일 이내에 이미 지급받은 재화 등의 대금을 환급합니다. 이 경우 “몰”이 이용자에게 재화등의 환급을 지연한때에는 그 지연기간에 대하여 「전자상거래 등에서의 소비자보호에 관한 법률 시행령」제21조의2에서 정하는 지연이자율을 곱하여 산정한 지연이자를 지급합니다.
...
① “몰”은 법령과 이 약관이 금지하거나 공서양속에 반하는 행위를 하지 않으며 이 약관이 정하는 바에 따라 지속적이고, 안정적으로 재화․용역, 구매대행 서비스를 제공하는데 최선을 다하여야 합니다.
...
※ 개인정보 수집 및 이용에 동의하지 않을 권리가 있으며, 동의 거부 시 일부 서비스 이용이 제한될 수 있습니다.
...
- 목적: 상품 재 입고 안내
- 항목: 휴대폰 번호
- 보유기간: 목적 달성 완료 시 즉시 삭제 (단, 내부방침 및 관계법령의 규정에 의하여 보존할 필요가 있는 경우 일정기간 동안 보관)

※ 개인정보 수집 및 이용에 동의하지 않을 권리가 있으며, 동의 거부 시 재입고 알림 신청이 불가합니다.''',
                                style: TextStyle(
                                  fontSize: getFontSize(context, FontSizeType.small),
                                ),
                              ),
                        ...
                        child: ExpansionTile(
                          title: Row(
                            children: [
                              ...
                              Checkbox(
                                value: isEmailMarketingAgreed,
                                onChanged: _toggleEmailMarketingAgreement,
                              ),
                            ],
                          ),
                          children: <Widget>[
...
※ 개인정보 수집 및 이용에 동의하지 않을 권리가 있으며, 동의 거부 시 일부 서비스 이용이 제한될 수 있습니다.
              ...
              Column(
                children: [
                  ...
                  CheckboxListTile(
                    title: Text(
                      "[선택] SMS로 마케팅 정보 수신에 동의합니다",
                      style: TextStyle(
                        fontSize: getFontSize(context, FontSizeType.small),
                      ),
                    ),
                    value: isSMSMarketingAgreed,
                    onChanged: _toggleSMSMarketingAgreement,
                  ),
                  CheckboxListTile(
                    title: Text(
                      "[전체 동의] 모든 항목에 동의합니다",
